                ...
                for target in targets:
                    try:
                        ...
                        for i in range(len(listid)):
                            getstory = config.igfunc.json.loads(config.igfunc.get_request(1000000000,useragent,'feed/user/'+str(listid[i])+'/story/',cookie).text)
                            for storyitem in getstory["reel"]["items"]:
                                sleep_1 = config.igfunc.random.randint(1,0) # Jeda per view story
                                sleep_2 = config.igfunc.random.randint(1,0) # Jeda per view story 1 akun user
                    ...
                    if len(todays) >= 2000000000000000:
                        print("[!] Limit Instagram API 200000000 seen/day")
